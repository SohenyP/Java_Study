...
알파벳과 그 개수가 모두 일치합니다. 
즉 어느 한 단어를 재 배열하면 상대편 단어가 될 수 있는 것을 
아나그램이라 합니다.
...
public class C01_32_01 {
	public String solution(String str1, String str2) {
		...
		for(char key : str1.toCharArray()) {
			...
			//key에 value를 담되, 기존key 있는 경우 대비
			map.put(key, map.getOrDefault(key, 0)+1);
		}
		//str2를 char[] 
		for(char c : str2.toCharArray()) {
			//map.conatins(c)가 없거나, map.get(c)가 값이 0일때
			if(!map.containsKey(c) || map.get(c) == 0) {
				//NO return
				return "NO";
			}
			//아니면 key가 c인 value를 하나씩 빼주기
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.print(T.solution(str1, str2)); // 값출력
	}
