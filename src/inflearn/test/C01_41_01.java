...
public class C01_41_01 {
	public int solution(int n, int k) {
		int ans = 0;
		Queue<Integer> q = new LinkedList<>(); //q선언
		for(int i = 1; i <= n; i ++) {
			q.offer(i);
		}
		while(!q.isEmpty()) {
			for(int i = 1; i < k; i++) { //k-1개만
				q.offer(q.poll()); // 꺼낸 값을 다시 넣어주기
			}
			q.poll(); //k번째 아웃!
			if(q.size() == 1) {
				ans = q.poll(); //한 명 왕자 번호 따오기 
			}
		}
		return ans;
	}
	...
	public static void main(String[] args) throws IOException {
		C01_41_01 T = new C01_41_01();

		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st = new StringTokenizer(br.readLine(), " "); 
		int n = Integer.parseInt(st.nextToken()); //왕자수
		int k = Integer.parseInt(st.nextToken()); //탈락 번호
