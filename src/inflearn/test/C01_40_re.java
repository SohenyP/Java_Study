	...
	public int solution(String str) {
		...
		for(int i = 0; i < str.length(); i++) {
			if(str.charAt(i) == '(') {
				...
				stack.pop(); //레이저니까 하나 빼
				if(str.charAt(i-1) == '(') { // (((이런게 있다! 그럼 얘는 이제 잘릴 막대니까
					ans += stack.size(); // (갯수만큼 더하기
				}
				else {
					ans++; //)이 나오므로써 막대기의 끝을 알리기에 막대기 1개 추가
				}
			}
		}
		return ans;
