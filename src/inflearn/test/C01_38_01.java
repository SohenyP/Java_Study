	...
	public int solution(int n, int[][] board, int m, int[] moves) {
		...
		for(int pos : moves) {
			for(int i = 0; i < board.length; i++) {
				if(board[i][pos-1] != 0) { //인형이 있다면 (moves 배열 값에서 -1해야 행 찾아감)
					int tmp = board[i][pos-1]; //위치 인형값 담기
					board[i][pos-1] = 0; //인형 자리 0으로 변경
					if(!stack.isEmpty() && tmp == stack.peek()) { //stack뭔가 있고, 인형번호 == stack맨위
						ans += 2; //같으면 터지니까 두개 더하기
						stack.pop(); //터졌으니 맨 윗값 빼기
					}
					else {
						stack.push(tmp); //비었거나, 같지 않다면 뽑은 인형 넣기
					}
					break;
				}
			}
		...
		public static void main(String[] args) throws IOException {
			C01_38_01 T = new C01_38_01();
	
			BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
			StringTokenizer st;
			int n = Integer.parseInt(br.readLine()); // board배열 길이
			...
			int m = Integer.parseInt(br.readLine()); // moves 길이
			int[] moves = new int[m];
